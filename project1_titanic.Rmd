...
The xgbTree model performed best, as according to cross validated accuracy. However, if we consider the one-standard-error rule (which argues that we should select the simplest model within one standard error of the best), we might choose the partial logistic model (i.e., the one that includes a subset of only significant predictors). Therefore, we will use partial_logistic for our final predictions.

NOTE: this might seem stupid, but I made submissions for each of the five models, and partial_logistic ended up being best!

### Predictions

```{r log_mod2 predictions}
